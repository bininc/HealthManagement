...
<id></id>
        ...
        public void Indata(string videoId)
        {
            if (!string.IsNullOrEmpty(videoId))
            {
                ...
                btnAdd.Text = "确定修改";
                isadd = videoId;
                string strmlx = TmoReomotingClient.InvokeServerMethod(funCode.GetVideoId, new object[] { videoId }).ToString();
        ...
        void btnAdd_Click(object sender, EventArgs e)
        {
            if (string.IsNullOrEmpty(isadd))
            {
              ...
              bool blt = (bool)TmoReomotingClient.InvokeServerMethod(funCode.AddVideo, new object[] { TmoShare.getXMLFromDataSet(ds) });
                if (blt)
                {
            ...
            else
            {
                ...
                bool blt = (bool)TmoReomotingClient.InvokeServerMethod(funCode.UpdateVideo, new object[] { TmoShare.getXMLFromDataSet(ds) });
                if (blt)
                {
