...
{
    public partial class FrmSleepShow : DevExpress.XtraEditors.XtraForm
    {
        #region 成员字段
        int _pageSize = 1;
        int _currentPage = 1;
        int currentDicPage = 1;
...
<mt_time></mt_time>
        ...
        #endregion
        public FrmSleepShow(string userID, string dicCode,string name)
        {
        ...
        public void GetData()
        {
            ...
            }, x =>
            {
                try
                {
                    ...
                    SetContorl(dt, _dicNamevalue,1,true);
        ...
        Dictionary<int, List<DataRow>> dicRows = new Dictionary<int, List<DataRow>>();
        ...
        private void linkeClick(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (this.currentDicPage < 12)
                linkNext.Enabled = true;
            if (currentDicPage <= 1)
            {
                linkUpNext.Enabled = false;
                return;
            }
            else
            {
                linkUpNext.Enabled = true;
                currentDicPage -= 1;
                SetContorl(null, _dicNamevalue, currentDicPage);

            }
        }

        private void llblNextClick(object sender, LinkLabelLinkClickedEventArgs e)
        {
            if (currentDicPage > 1)
                linkUpNext.Enabled = true;
            if (this.currentDicPage >=12)
