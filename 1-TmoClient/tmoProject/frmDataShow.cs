...
namespace tmoProject
{
  ...
  <mt_time></mt_time>
        ...
        public void GetItemData()
        {
            ...
            string strmlx = TmoReomotingClient.InvokeServerMethodT<string>(funCode.GetItemData, new object[] { "" }).ToString();
                ...
                try
                {
                    ...
                    else
                    {
                        if (_dicCode == "99")
                            ...
                            _pageSize = 5;
                        string strmlx = TmoReomotingClient.InvokeServerMethodT<string>(funCode.GetMonitorData, new object[] { selexml }).ToString();
                        _dsGetDataResult = TmoShare.getDataSetFromXML(strmlx);
                        if (TmoShare.DataSetIsNotEmpty(_dsGetDataResult))
                        {
                            DataTable dtCount = _dsGetDataResult.Tables["Count"];
