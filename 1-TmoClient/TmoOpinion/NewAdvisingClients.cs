...
{
    public partial class NewAdvisingClients : UCBase
    {
        #region member
        int _pageSize = 100;
        int _currentPage = 1;
...
@"<tmo>
   <user_id></user_id>
    <user_time></user_time>
        ...
        void askButton_Click(object sender, EventArgs e)
        {
            DialogResult dr = new NewFrmAdviseOpinion().ShowDialog();
        ...
        void gridView1_RowCellClick(object sender, DevExpress.XtraGrid.Views.Grid.RowCellClickEventArgs e)
        {
            ...
            {
                if (drDel["is_reply"].ToString() == "已回复")
                {
                    DXMessageBox.Show("该问题已经回复！", true);
                }
                else
                {
                    var adviseID = drDel["con_id"];
                    var adviseContent = drDel["con_content"];
                    NewFrmAdviseAnswer frmda = new NewFrmAdviseAnswer();
                    frmda.indata(adviseID.ToString(), adviseContent.ToString(), TmoComm.login_docInfo.doc_id.ToString(),drDel["we_id"].ToString());
                    frmda.ShowDialog();
        ...
        public void GetData()
        {
            this.ShowWaitingPanel(() =>
            {
                ...
                try
                {
                    ...
                    string strmlx = TmoReomotingClient.InvokeServerMethod(funCode.GetNewOpinionData, new object[] { selexml }).ToString();
