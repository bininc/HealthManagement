   ...
   <user_id></user_id>
    <user_time></user_time>
        ...
        public void indata(DataRow dr)
        {
            ...
            try
            {
                bool isIn = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethod(funCode.reportUpdate, new object[] { userID, user_times, "hbxxg" });
                if (!isIn)
                {
                    bool issuccess = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethod(funCode.reportIn, new object[] { userID, user_times, "糖尿病并发心血管病", JianYi, JieLun, "hbxxg" });
                }
            }
            catch (Exception)
            {
        ...
        public void RefData(string userId, string user_times, string quesid)
        {
            ...
            string resultxml = TmoLinkServer.TmoReomotingClient.InvokeServerMethod(funCode.getFeiPang, new object[] { userId, user_times, quesid }).ToString();
