   ...
   <user_id></user_id>
    <user_time></user_time>
        ...
        public void indata(DataRow dr)
        {
            ...
            try
            {
                bool isIn = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethod(funCode.reportUpdate, new object[] { userID, user_times, "hbnxb" });
                if (!isIn)
                {
                    bool issuccess = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethod(funCode.reportIn, new object[] { userID, user_times, "糖尿病并发脑血管病", JianYi, JieLun, "hbnxb" });
                }
            }
            catch (Exception)
            {
