    ...
    <user_time></user_time>
       ...
       public Dengji()
        ...
        public void indata(DataRow dr,string ids)
        {
            ...
            try
            {
                bool isIn = (bool)TmoLinkServer.TmoServiceClient.InvokeServerMethodT<bool>(funCode.reportUpdate, new object[] { userID, user_times, "tangniaobing" });
                if (!isIn)
                {
                    bool issuccess = (bool)TmoLinkServer.TmoServiceClient.InvokeServerMethodT<bool>(funCode.reportIn, new object[] { userID, user_times, "糖尿病", JianYi, JieLun, "tangniaobing" });
                }
            }
            catch (Exception)
            {
