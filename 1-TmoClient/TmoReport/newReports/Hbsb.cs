   ...
   <user_id></user_id>
    <user_time></user_time>
        ...
        public void indata(DataRow dr)
        {
            ...
            try
            {
                bool isIn = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethodT<bool>(funCode.reportUpdate, new object[] { userID, user_times, "hbsb" });
                if (!isIn)
                {
                    bool issuccess = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethodT<bool>(funCode.reportIn, new object[] { userID, user_times, "糖尿病并发肾病", JianYi, JieLun, "hbsb" });
                }
            }
            catch (Exception)
            {
