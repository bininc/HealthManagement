   ...
   <user_id></user_id>
    <user_time></user_time>
        ...
        public void indata(DataRow dr,string ccid,string xuezhiYi)
        {
            ...
            try
            {
                bool isIn = (bool)TmoLinkServer.TmoServiceClient.InvokeServerMethodT<bool>(funCode.reportUpdate, new object[] { userID, user_times, "gaoxueya" });
                if (!isIn)
                {
                    bool issuccess = (bool)TmoLinkServer.TmoServiceClient.InvokeServerMethodT<bool>(funCode.reportIn, new object[] { userID, user_times, "高血压", JianYi, JieLun, "gaoxueya" });
                }
            }
            catch (Exception)
            {
