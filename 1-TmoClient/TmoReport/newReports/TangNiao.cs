   ...
   <user_id></user_id>
    <user_time></user_time>
        ...
        public void indata(DataRow dr,string ccid,string xuezhiYi)
        {
            ...
            try
            {
                bool isIn = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethodT<bool>(funCode.reportUpdate, new object[] { userID, user_times, "gaoxueya" });
                if (!isIn)
                {
                    bool issuccess = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethodT<bool>(funCode.reportIn, new object[] { userID, user_times, "高血压", JianYi, JieLun, "gaoxueya" });
                }
            }
            catch (Exception)
            {
        ...
        public void getData(string userId, string user_times, string quesTypeId,int age)
        {
            ...
            shenHuo.Add("长期过量饮酒");
            string ssHeng = "无";
            string resultxml = TmoLinkServer.TmoReomotingClient.InvokeServerMethodT<string>(funCode.getTest, new object[] { userId, user_times, quesTypeId }).ToString();
            DataTable DsReslut = TmoShare.getDataTableFromXML(resultxml);
            string resultxml1 = TmoLinkServer.TmoReomotingClient.InvokeServerMethodT<string>(funCode.getScreenData, new object[] { userId, user_times, "'02390D277242464192B05F08D03D298B','751624508AA94D05864287B3CB6B723B'" }).ToString();
