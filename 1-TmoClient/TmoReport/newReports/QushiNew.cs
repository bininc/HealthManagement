...
{
    public partial class QushiNew : DevExpress.XtraReports.UI.XtraReport
    {
        DataSet dsadd = null;
        public QushiNew(DataSet advicDs)
        {
            dsadd = advicDs;
            InitializeComponent();
        }
        public void indata(DataRow dr)
        {
            ...
            if (TmoCommon.TmoShare.DataSetIsNotEmpty(dsadd))
            {
                if (dsadd.Tables[0] != null)
                {
                    if (dsadd.Tables[0].Rows.Count > 0)
                    {
                        DataRow dar = dsadd.Tables[0].Rows[0];
                      xuetangYin.Text = dar["sugar_reason"] == null ? "" : dar["sugar_reason"].ToString();
                      xutangJian.Text = dar["sugar_advice"] == null ? "" : dar["sugar_advice"].ToString();
                      xueyaYin.Text = dar["pressure_reason"] == null ? "" : dar["pressure_reason"].ToString();
                      xueyaJian.Text = dar["pressure_advice"] == null ? "" : dar["pressure_advice"].ToString();
                     xuezhiYin.Text = dar["bloodlipid_reason"] == null ? "" : dar["bloodlipid_reason"].ToString();
                     xuezhiJian.Text = dar["bloodlipid_advice"] == null ? "" : dar["bloodlipid_advice"].ToString();
                     jiankangshuzhi.Text = dar["jiankangshuzhi"] == null ? "" : dar["jiankangshuzhi"].ToString();
                     genzhongzuocheng.Text = dar["genzong"] == null ? "" : dar["genzong"].ToString();
                    }
                }
            }
