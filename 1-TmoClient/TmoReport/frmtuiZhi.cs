...
namespace TmoReport
{
    public partial class frmtuiZhi : DevExpress.XtraEditors.XtraForm
    {
        ...
        public void GetData() {
            ...
            DataSet ds = TmoCommon.TmoShare.getDataSetFromXML(TmoLinkServer.TmoReomotingClient.InvokeServerMethod(TmoCommon.funCode.GettuiDataUser, new object[] { userIDDD,userTimesss }).ToString());
            btnsave.Text = "修改";
            if (!TmoShare.DataSetIsNotEmpty(ds))
            {
                btnsave.Text = "添加";
                ds = TmoCommon.TmoShare.getDataSetFromXML(TmoLinkServer.TmoReomotingClient.InvokeServerMethod(TmoCommon.funCode.getTuiData, new object[] { "" }).ToString());
            }
            if (TmoShare.DataSetIsNotEmpty(ds))
            {
        ...
        void btnsave_Click(object sender, EventArgs e)
        {
            ...
            if (btnsave.Text == "修改")
            {
                ...
                bool isT = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethod(TmoCommon.funCode.TuijianUpdate, new object[] { xmldata });
                if (isT)
                ...
                bool isT = (bool)TmoLinkServer.TmoReomotingClient.InvokeServerMethod(TmoCommon.funCode.TuijianZhi, new object[] { xmldata });
                if (isT)
