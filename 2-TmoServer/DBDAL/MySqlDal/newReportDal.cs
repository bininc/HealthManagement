...
{
    public class newReportDal : InewReport
    {
        ...
        {
            string sql = "select count(1) from tmo_conclusion where user_id='" + userId + "' and user_times='" + user_times + "' and reportName='" + reportName + "'";
        ...
        public bool reportIn(string userId, string user_times, string con_all, string advice, string conclusion, string reportName)
        {
            ...
            string sql = "insert into tmo_conclusion (user_id,id,user_times,cons_all,advice,conclusion,input_time,reportName) VALUES('" + userId + "','" + Guid.NewGuid().ToString().Replace("-", "") + "','"+user_times+"','"+con_all+"','"+advice+"','"+conclusion+"','"+DateTime.Now.ToString()+"','"+reportName+"')";
            int o = MySQLHelper.ExecuteSql(sql);
           if(o>0)
               return true;
            else
                return false;
